...
@export var flight_speed: float = 400.0  # REDUCED speed to make it more visible
@export var rotation_speed: float = 5.0  # REDUCED rotations per second to make it more visible
...
func _ready() -> void:
    ...
    # Make boomerang MUCH larger for better visibility
    ...
    # Create a simple colored shape regardless of texture loading
    # This way it will always be visible even if texture fails
    var texture_size = 50
    var img = Image.create(texture_size, texture_size, false, Image.FORMAT_RGBA8)
    img.fill(Color(1.0, 0.1, 0.1, 1.0))  # Pure bright red
    
    # Draw a more visible shape
    for x in range(texture_size):
        for y in range(texture_size):
            var dist = Vector2(x - texture_size/2, y - texture_size/2).length()
            if dist < texture_size/2 - 2 and dist > texture_size/4:
                img.set_pixel(x, y, Color(1.0, 0.1, 0.1, 1.0))  # Bright red
            else:
                img.set_pixel(x, y, Color(1.0, 1.0, 0.2, 0.2))  # Yellow center
    
    var fallback_texture = ImageTexture.create_from_image(img)
    ...
    sprite.modulate = Color(1.0, 0.2, 0.2, 1.0)  # Very bright red
...
func _physics_process(delta: float) -> void:
    ...
    # Rotate the boomerang slower to make it more visible
    sprite.rotation += rotation_speed * delta * 2 * PI
    
    # Ensure visibility and color stays consistent
    visible = true
    sprite.modulate = Color(1.0, 0.2, 0.2, 1.0)  # Keep it bright red
...
func _on_body_entered(body: Node) -> void:
    if body is Player and not has_hit_player:
        ...
        sprite.modulate = Color(1.0, 1.0, 0.1, 1.0)  # Bright yellow flash on collision
